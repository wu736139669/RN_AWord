...
export default class Info extends Component{
  constructor(props) {
    ...
    // console.log(this.props.navComponent);
    // this.props.navBarTintColor = 'red';
    this.props.navComponent.setNavItems({
      // leftItem: {
      //   component: (
      //     <TouchableOpacity style={[styles.navItem, {marginRight: 7}]}>
      //       <Image style={{width: 20, height: 20, backgroundColor:'red'}} source={{uri: shareImg}}/>
      //     </TouchableOpacity>
      //   ),
  ...
  componentDidMount(){
    storage.load({
              key: 'moviec',
                ...
                if (ret) {ret.push(this.props.movie.id)}else{
                  ret = [];
                  ret.push(this.props.movie.id);
                }

                console.log(ret);
                storage.save({
                 key: 'moviec',  //注意:请不要在key中使用_下划线符号!
                rawData: ret,
                expires: null
               });
                 }).catch( err => {
